# LiveKit AgentServer Architecture Diagram - Food Concierge Voice

## Overview

This document explains how the **LiveKit AgentServer framework** powers the Food Court Concierge voice experience. It covers the complete flow from voice input to voice output, showing how LiveKit, Python agents, OpenAI, and Supabase work together for real-time voice interactions.

> **Note**: This architecture supports optional **LemonSlice voice avatar** integration. When enabled, an animated avatar joins as a video participant with automatic lip-sync. See [docs/LEMONSLICE.md](LEMONSLICE.md) for setup details.

**Target Audience**: Beginner to Intermediate developers
**Implementation**: 
- Python Agent: `/agents/food_concierge_agentserver.py`
- Database Layer: `/agents/database.py`
- Token Endpoint: `/app/api/livekit-agentserver/token`
- Frontend: `/app/food/concierge-agentserver`

---

## High-Level Architecture

```
┌──────────────────┐
│   User Voice     │  (Speaks into microphone)
│  React Frontend  │
└────────┬─────────┘
         │ WebRTC Audio Stream
         ▼
┌─────────────────────────────────────────────────────────┐
│  LiveKit Cloud Infrastructure                           │
│  - WebRTC Media Server                                  │
│  - Room Management                                      │
│  - Agent Dispatch System                                │
└────────┬────────────────────────────────────────────────┘
         │ RTC Session
         ▼
┌─────────────────────────────────────────────────────────┐
│  Python AgentServer                                     │
│  food_concierge_agentserver.py                          │
│                                                         │
│  STT (Deepgram) → LLM (OpenAI) → TTS (OpenAI)          │
│  + Function Tools (9 tools)                             │
└────────┬────────────────────────────────────────────────┘
         │
         ├──────► OpenAI API (GPT-4o-mini)
         │        └─► Function calling for tools
         │
         └──────► database.py → Supabase
                  └─► Query food data
```

---

## Core Components

### 1. Connection Flow

```
User clicks "Start Voice"
       │
       ▼
┌─────────────────────────────────────────────┐
│  Frontend: POST /api/livekit-agentserver/   │
│            token                            │
│                                             │
│  Request: {                                 │
│    roomName: "food-concierge-agentserver-   │
│               {timestamp}",                 │
│    participantName: "user-{random}"         │
│  }                                          │
└─────────────────────────────────────────────┘
       │
       ▼
┌─────────────────────────────────────────────┐
│  Token Endpoint                             │
│  /app/api/livekit-agentserver/token/        │
│  route.ts                                   │
│                                             │
│  1. Create room in LiveKit                  │
│  2. Dispatch agent: "ubereats-food-         │
│     concierge"                              │
│  3. Generate access token                   │
│  4. Return to frontend                      │
└─────────────────────────────────────────────┘
       │
       ▼
┌─────────────────────────────────────────────┐
│  Frontend Connects                          │
│  @livekit/components-react                  │
│                                             │
│  • LiveKitRoom component                    │
│  • Connects with token                      │
│  • Publishes microphone audio               │
│  • Subscribes to agent audio                │
└─────────────────────────────────────────────┘
       │
       ▼
┌─────────────────────────────────────────────┐
│  Python Agent Auto-Joins                    │
│  @server.rtc_session decorator              │
│                                             │
│  • Agent dispatched by LiveKit              │
│  • Joins same room                          │
│  • Starts STT/LLM/TTS pipeline              │
│  • Sends greeting via TTS                   │
└─────────────────────────────────────────────┘
       │
       ▼
┌─────────────────────────────────────────────┐
│  Voice Conversation Begins                  │
│  Real-time WebRTC bidirectional audio       │
└─────────────────────────────────────────────┘
```

---

## AgentServer Pattern

### Architecture Overview

```
┌────────────────────────────────────────────────────────────┐
│  LiveKit AgentServer (Python)                              │
│  ═══════════════════════════════════════════               │
│                                                            │
│  @server.rtc_session decorator                             │
│  └─► Handles room join events automatically                │
│                                                            │
│  AgentSession[UserState]                                   │
│  ├─► STT: inference.STT("deepgram/nova-3")                 │
│  ├─► LLM: inference.LLM("openai/gpt-4o-mini")              │
│  ├─► TTS: openai.TTS()                                     │
│  ├─► VAD: silero.VAD.load()                                │
│  └─► Tools: 9 function_tool decorators                     │
│                                                            │
│  UserState (typed context)                                 │
│  ├─► user_id                                               │
│  ├─► cart_id                                               │
│  ├─► profile                                               │
│  └─► local_participant (for data channel)                  │
└────────────────────────────────────────────────────────────┘
```

### Key Differences from AI SDK

| Aspect | AI SDK (Chat) | LiveKit (Voice) |
|--------|---------------|-----------------|
| **Transport** | HTTP/POST | WebRTC |
| **Input** | Text messages | Audio stream |
| **Output** | Text stream | Audio stream |
| **Model Access** | Direct OpenAI API | Via LiveKit inference layer |
| **Tool Definition** | `tool()` from `ai` | `@function_tool` decorator |
| **State Management** | Message history | UserState dataclass |
| **Runtime** | Node.js/Edge | Python |
| **Real-time** | Streaming text | Streaming audio |

---

## Voice Conversation Flow - Complete Pipeline

```
User speaks: "Show me Thai restaurants"
       │
       ▼
┌──────────────────────────────────────────────────────────┐
│  1. AUDIO CAPTURE                                        │
├──────────────────────────────────────────────────────────┤
│  Frontend (Browser)                                      │
│  • Microphone captures audio                             │
│  • WebRTC encodes audio stream                           │
│  • Sends to LiveKit Cloud via WebRTC                     │
└──────────────────────────────────────────────────────────┘
       │
       ▼
┌──────────────────────────────────────────────────────────┐
│  2. SPEECH-TO-TEXT (STT)                                 │
├──────────────────────────────────────────────────────────┤
│  Python Agent: AgentSession                              │
│                                                          │
│  STT: inference.STT("deepgram/nova-3")                   │
│  • Receives audio chunks from LiveKit                    │
│  • Sends to Deepgram Nova-3 model                        │
│  • Returns transcription                                 │
│                                                          │
│  Output: "Show me Thai restaurants"                      │
│                                                          │
│  Event: user_speech_committed                            │
│  └─► Logged and sent to frontend                         │
└──────────────────────────────────────────────────────────┘
       │
       ▼
┌──────────────────────────────────────────────────────────┐
│  3. LANGUAGE MODEL (LLM)                                 │
├──────────────────────────────────────────────────────────┤
│  Python Agent: AgentSession                              │
│                                                          │
│  LLM: inference.LLM("openai/gpt-4o-mini")                │
│  • Receives transcript from STT                          │
│  • Analyzes with system instructions                     │
│  • Decides action: text OR function call                 │
│                                                          │
│  System Instructions:                                    │
│  "You are the Food Concierge Assistant..."               │
│                                                          │
│  Available Functions (9 tools):                          │
│  - get_user_profile                                      │
│  - find_food_item                                        │
│  - find_restaurants_by_type                              │
│  - get_restaurant_menu                                   │
│  - fetch_menu_item_image                                 │
│  - quick_view_cart                                       │
│  - quick_add_to_cart                                     │
│  - remove_from_cart                                      │
│  - update_cart_quantity                                  │
│  - quick_checkout                                        │
│                                                          │
│  Decision: Call find_restaurants_by_type                 │
│  Parameters: { cuisine_type: "Thai" }                    │
│                                                          │
│  Event: function_calls_collected                         │
│  └─► Logged to terminal                                  │
└──────────────────────────────────────────────────────────┘
       │
       ▼
┌──────────────────────────────────────────────────────────┐
│  4. FUNCTION TOOL EXECUTION                              │
├──────────────────────────────────────────────────────────┤
│  Python Agent: FoodConciergeAgent                        │
│                                                          │
│  @function_tool                                          │
│  async def find_restaurants_by_type_tool(               │
│      ctx: RunContext[UserState],                         │
│      cuisine_type: str                                   │
│  ) -> str:                                               │
│                                                          │
│  1. Log: "🔧 Tool: find_restaurants_by_type(            │
│           cuisine_type='Thai')"                          │
│                                                          │
│  2. Call database layer:                                 │
│     results = await search_restaurants_by_cuisine(       │
│         "Thai", 3                                        │
│     )                                                    │
└──────────────────────────────────────────────────────────┘
       │
       ▼
┌──────────────────────────────────────────────────────────┐
│  5. DATABASE QUERY                                       │
├──────────────────────────────────────────────────────────┤
│  database.py: search_restaurants_by_cuisine()            │
│                                                          │
│  Supabase Query:                                         │
│  ─────────────────────────────────────────────           │
│  supabase.table("fc_restaurants")                        │
│    .select(                                              │
│       "id, slug, name, cuisine, rating,                  │
│        eta_minutes, delivery_fee, ..."                   │
│    )                                                     │
│    .eq("is_active", True)                                │
│    .or_(                                                 │
│       "cuisine.ilike.%Thai%,                             │
│        cuisine_group.ilike.%Thai%,                       │
│        name.ilike.%Thai%"                                │
│    )                                                     │
│    .order("name")                                        │
│    .limit(5)                                             │
│    .execute()                                            │
│                                                          │
│  Returns: [                                              │
│    {                                                     │
│      id: "uuid",                                         │
│      name: "Bangkok Express",                            │
│      cuisine: "Thai",                                    │
│      rating: 4.5,                                        │
│      etaMinutes: 25,                                     │
│      deliveryFee: 2.99                                   │
│    },                                                    │
│    ...                                                   │
│  ]                                                       │
└──────────────────────────────────────────────────────────┘
       │
       ▼
┌──────────────────────────────────────────────────────────┐
│  6. SEND DATA TO FRONTEND                                │
├──────────────────────────────────────────────────────────┤
│  Python Agent Tool                                       │
│                                                          │
│  if ctx.userdata.local_participant:                      │
│      tool_data = {                                       │
│          "type": "tool_call",                            │
│          "tool_name": "find_restaurants_by_type",        │
│          "result": {                                     │
│              "restaurants": results,                     │
│              "cuisine": "Thai"                           │
│          }                                               │
│      }                                                   │
│      await ctx.userdata.local_participant.publish_data(  │
│          json.dumps(tool_data).encode(),                 │
│          reliable=True                                   │
│      )                                                   │
│                                                          │
│  Purpose: Send structured data for card rendering        │
│  └─► Frontend displays restaurant cards                  │
└──────────────────────────────────────────────────────────┘
       │
       ▼
┌──────────────────────────────────────────────────────────┐
│  7. TOOL RETURNS TEXT SUMMARY                            │
├──────────────────────────────────────────────────────────┤
│  Python Agent Tool                                       │
│                                                          │
│  response_parts = [                                      │
│      f"Found {len(results)} Thai restaurants:",          │
│      "1. Bangkok Express - Rating: 4.5",                 │
│      "2. Thai Orchid - Rating: 4.7",                     │
│      "3. Spicy Basil - Rating: 4.3"                      │
│  ]                                                       │
│  return "\n".join(response_parts)                        │
│                                                          │
│  Event: function_calls_finished                          │
│  └─► Tool result logged                                  │
└──────────────────────────────────────────────────────────┘
       │
       ▼
┌──────────────────────────────────────────────────────────┐
│  8. LLM GENERATES NATURAL RESPONSE                       │
├──────────────────────────────────────────────────────────┤
│  Python Agent: AgentSession LLM                          │
│                                                          │
│  Input: Tool result text                                 │
│  Processing: Convert to natural language                 │
│                                                          │
│  Output: "I found 3 great Thai restaurants for you:     │
│           Bangkok Express has a 4.5 rating and can       │
│           deliver in 25 minutes. Thai Orchid is rated    │
│           4.7 stars. And Spicy Basil is 4.3 stars.       │
│           Which one sounds good?"                        │
└──────────────────────────────────────────────────────────┘
       │
       ▼
┌──────────────────────────────────────────────────────────┐
│  9. TEXT-TO-SPEECH (TTS)                                 │
├──────────────────────────────────────────────────────────┤
│  Python Agent: AgentSession                              │
│                                                          │
│  TTS: openai.TTS()                                       │
│  • Receives text from LLM                                │
│  • Sends to OpenAI TTS API                               │
│  • Returns audio stream                                  │
│                                                          │
│  Event: agent_speech_committed                           │
│  └─► Text logged and sent to frontend                    │
└──────────────────────────────────────────────────────────┘
       │
       ▼
┌──────────────────────────────────────────────────────────┐
│  10. AUDIO PLAYBACK                                      │
├──────────────────────────────────────────────────────────┤
│  LiveKit Cloud → Frontend                                │
│                                                          │
│  • Agent publishes audio track                           │
│  • LiveKit streams via WebRTC                            │
│  • Frontend's <AudioTrack> plays audio                   │
│  • User hears: "I found 3 great Thai restaurants..."     │
└──────────────────────────────────────────────────────────┘
       │
       ▼
┌──────────────────────────────────────────────────────────┐
│  CONVERSATION CONTINUES...                               │
│  User can respond immediately (full duplex)              │
└──────────────────────────────────────────────────────────┘
```

---

## Detailed Action Flows

## Action 1: Search Restaurants (Voice)

```
User says: "Show me Thai restaurants"
       │
       ▼
┌──────────────────────────────────────────────────────────┐
│  STT: Deepgram Nova-3                                    │
│  Transcribes: "Show me Thai restaurants"                 │
└──────────────────────────────────────────────────────────┘
       │
       ▼
┌──────────────────────────────────────────────────────────┐
│  LLM: OpenAI GPT-4o-mini                                 │
│                                                          │
│  Analyzes intent: User wants restaurant search           │
│  Selected tool: find_restaurants_by_type                 │
│  Extracted params: { cuisine_type: "Thai" }              │
└──────────────────────────────────────────────────────────┘
       │
       ▼
┌──────────────────────────────────────────────────────────┐
│  Tool Execution: find_restaurants_by_type_tool()         │
│                                                          │
│  async def find_restaurants_by_type_tool(                │
│      ctx: RunContext[UserState],                         │
│      cuisine_type: "Thai"                                │
│  ):                                                      │
│      max_results = 3  # hardcoded                        │
│      results = await search_restaurants_by_cuisine(      │
│          "Thai", 3                                       │
│      )                                                   │
└──────────────────────────────────────────────────────────┘
       │
       ▼
┌──────────────────────────────────────────────────────────┐
│  Database Query (database.py)                            │
│                                                          │
│  async def search_restaurants_by_cuisine(                │
│      cuisine_type: str,                                  │
│      max_results: int                                    │
│  ):                                                      │
│      response = supabase                                 │
│          .table("fc_restaurants")                        │
│          .select("*")                                    │
│          .eq("is_active", True)                          │
│          .or_(                                           │
│              "cuisine.ilike.%Thai%,"                     │
│              "cuisine_group.ilike.%Thai%,"               │
│              "name.ilike.%Thai%"                         │
│          )                                               │
│          .order("name")                                  │
│          .limit(5)                                       │
│          .execute()                                      │
│                                                          │
│      # Ensure images from Pexels if missing              │
│      for each restaurant:                                │
│          if not hero_image:                              │
│              hero_image = await fetch_image_from_pexels( │
│                  restaurant_name + " food restaurant"    │
│              )                                           │
└──────────────────────────────────────────────────────────┘
       │
       ▼
┌──────────────────────────────────────────────────────────┐
│  Data Channel to Frontend                                │
│                                                          │
│  await local_participant.publish_data(                   │
│      json.dumps({                                        │
│          "type": "tool_call",                            │
│          "tool_name": "find_restaurants_by_type",        │
│          "result": {                                     │
│              "restaurants": [                            │
│                  {                                       │
│                      "id": "uuid",                       │
│                      "name": "Bangkok Express",          │
│                      "cuisine": "Thai",                  │
│                      "rating": 4.5,                      │
│                      "etaMinutes": 25,                   │
│                      "heroImage": "https://..."          │
│                  },                                      │
│                  ...                                     │
│              ],                                          │
│              "cuisine": "Thai"                           │
│          }                                               │
│      }).encode(),                                        │
│      reliable=True                                       │
│  )                                                       │
│                                                          │
│  Purpose: Frontend renders restaurant cards              │
└──────────────────────────────────────────────────────────┘
       │
       ▼
┌──────────────────────────────────────────────────────────┐
│  Tool Returns Text                                       │
│                                                          │
│  return """Found 3 Thai restaurants:                     │
│  1. Bangkok Express - Rating: 4.5                        │
│  2. Thai Orchid - Rating: 4.7                            │
│  3. Spicy Basil - Rating: 4.3"""                         │
└──────────────────────────────────────────────────────────┘
       │
       ▼
┌──────────────────────────────────────────────────────────┐
│  LLM Generates Response                                  │
│                                                          │
│  "I found 3 great Thai restaurants for you. Bangkok      │
│   Express has a 4.5 star rating and can deliver in       │
│   25 minutes. Thai Orchid is highly rated at 4.7.        │
│   And Spicy Basil has 4.3 stars. Which one would         │
│   you like to explore?"                                  │
└──────────────────────────────────────────────────────────┘
       │
       ▼
┌──────────────────────────────────────────────────────────┐
│  TTS: OpenAI TTS                                         │
│  Converts text to audio stream                           │
│  User hears the response                                 │
└──────────────────────────────────────────────────────────┘
```

**Key Tables**: `fc_restaurants`
**External APIs**: Pexels (for images)
**Data Channel**: Restaurant cards sent to frontend

---

## Action 2: Search Menu Items (Voice)

```
User says: "Find me some cheesecake"
       │
       ▼
┌──────────────────────────────────────────────────────────┐
│  STT → LLM → Tool Selection                              │
│                                                          │
│  Tool: find_food_item                                    │
│  Params: { query: "cheesecake" }                         │
└──────────────────────────────────────────────────────────┘
       │
       ▼
┌──────────────────────────────────────────────────────────┐
│  Database Query (database.py)                            │
│                                                          │
│  async def search_menu_items(query, max_results):        │
│      # Multi-word search enhancement                     │
│      words = query.strip().split()                       │
│      # "New York cheesecake" → ["New", "York",           │
│      #  "cheesecake"]                                    │
│                                                          │
│      # Build OR conditions for each word                 │
│      for word in words:                                  │
│          if len(word) >= 3:                              │
│              filters.append(                             │
│                  f"name.ilike.%{word}%,"                 │
│                  f"description.ilike.%{word}%"           │
│              )                                           │
│                                                          │
│      response = supabase                                 │
│          .table("fc_menu_items")                         │
│          .select("""                                     │
│              id, slug, name, description,                │
│              base_price, tags, calories, image,          │
│              section:section_id(id, name),               │
│              restaurant:restaurant_id(id, slug, name)    │
│          """)                                            │
│          .eq("is_available", True)                       │
│          .or_(/* multi-word OR conditions */)            │
│          .order("name")                                  │
│          .limit(max_results * 5)                         │
│          .execute()                                      │
│                                                          │
│      # Rank results by word match count                  │
│      def score_item(item):                               │
│          text = f"{item['name']} {item['description']}"  │
│          return sum(1 for word in words                  │
│                     if word.lower() in text.lower())     │
│                                                          │
│      results.sort(key=score_item, reverse=True)          │
│                                                          │
│      # Fetch images from Pexels if missing               │
│      for item in results:                                │
│          if not item.get("image"):                       │
│              item["image"] = await ensure_menu_item_     │
│                  image(item_id, item_slug, item_name,    │
│                         restaurant_name)                 │
└──────────────────────────────────────────────────────────┘
       │
       ▼
┌──────────────────────────────────────────────────────────┐
│  Returns: [                                              │
│      {                                                   │
│          "id": "item-123",                               │
│          "name": "Classic New York Cheesecake",          │
│          "price": 8.99,                                  │
│          "description": "Creamy cheesecake...",          │
│          "restaurantName": "Dessert Palace",             │
│          "restaurantSlug": "dessert-palace",             │
│          "image": "https://images.pexels.com/..."        │
│      },                                                  │
│      {                                                   │
│          "name": "Tropical Cheesecake",                  │
│          "price": 9.99,                                  │
│          "restaurantName": "Island Breeze",              │
│          ...                                             │
│      }                                                   │
│  ]                                                       │
└──────────────────────────────────────────────────────────┘
       │
       ▼
┌──────────────────────────────────────────────────────────┐
│  Data Channel: Send results to frontend                  │
│  Frontend displays menu item cards with images           │
└──────────────────────────────────────────────────────────┘
       │
       ▼
┌──────────────────────────────────────────────────────────┐
│  Voice Response                                          │
│                                                          │
│  "I found 2 cheesecake options:                          │
│   1. Classic New York Cheesecake from Dessert Palace     │
│      for $8.99                                           │
│   2. Tropical Cheesecake from Island Breeze for $9.99.   │
│   Would you like to add one to your cart?"               │
└──────────────────────────────────────────────────────────┘
```

**Key Tables**: `fc_menu_items`, `fc_menu_sections`, `fc_restaurants`
**Features**:
- Multi-word search (handles "New York cheesecake")
- Ranking by relevance
- Auto-image fetching from Pexels

---

## Action 3: Add Item to Cart (Voice)

```
User says: "Add the New York cheesecake to my cart"
       │
       ▼
┌──────────────────────────────────────────────────────────┐
│  STT → LLM → Tool Selection                              │
│                                                          │
│  Tool: quick_add_to_cart                                 │
│  Params: {                                               │
│      item_name: "New York cheesecake",                   │
│      quantity: "1"  # inferred from context              │
│  }                                                       │
└──────────────────────────────────────────────────────────┘
       │
       ▼
┌──────────────────────────────────────────────────────────┐
│  Tool Execution: quick_add_to_cart_tool()                │
│                                                          │
│  1. Parse quantity: int("1") = 1                         │
│  2. Call database layer:                                 │
│     result = add_to_voice_cart(                          │
│         item_name="New York cheesecake",                 │
│         quantity=1                                       │
│     )                                                    │
└──────────────────────────────────────────────────────────┘
       │
       ▼
┌──────────────────────────────────────────────────────────┐
│  Database Operations (database.py)                       │
│                                                          │
│  def add_to_voice_cart(item_name, quantity):             │
│      global voice_cart  # In-memory cart                 │
│                                                          │
│      # Step 1: Search for item in database               │
│      response = supabase                                 │
│          .table("fc_menu_items")                         │
│          .select("""                                     │
│              id, slug, name, base_price,                 │
│              restaurant:restaurant_id(                   │
│                  id, slug, name, delivery_fee            │
│              )                                           │
│          """)                                            │
│          .ilike("name", f"%{item_name}%")                │
│          .eq("is_available", True)                       │
│          .limit(1)                                       │
│          .execute()                                      │
│                                                          │
│      if not response.data:                               │
│          return {                                        │
│              "success": False,                           │
│              "message": "Item not found"                 │
│          }                                               │
│                                                          │
│      item = response.data[0]                             │
│      restaurant = item["restaurant"]                     │
│                                                          │
│      # Step 2: Initialize cart if doesn't exist          │
│      if voice_cart is None:                              │
│          voice_cart = {                                  │
│              "restaurantId": restaurant["id"],           │
│              "restaurantSlug": restaurant["slug"],       │
│              "restaurantName": restaurant["name"],       │
│              "items": [],                                │
│              "subtotal": 0,                              │
│              "deliveryFee": restaurant["delivery_fee"]   │
│                             or 2.99,                     │
│              "total": 0                                  │
│          }                                               │
│                                                          │
│      # Step 3: Check if item already in cart             │
│      existing_item = None                                │
│      for cart_item in voice_cart["items"]:               │
│          if cart_item["name"].lower() ==                 │
│             item["name"].lower():                        │
│              existing_item = cart_item                   │
│              break                                       │
│                                                          │
│      if existing_item:                                   │
│          # Update quantity                               │
│          existing_item["quantity"] += quantity           │
│          existing_item["totalPrice"] = (                 │
│              existing_item["basePrice"] *                │
│              existing_item["quantity"]                   │
│          )                                               │
│      else:                                               │
│          # Add new item                                  │
│          voice_cart["items"].append({                    │
│              "id": item["id"],                           │
│              "slug": item["slug"],                       │
│              "name": item["name"],                       │
│              "quantity": quantity,                       │
│              "basePrice": item["base_price"],            │
│              "totalPrice": item["base_price"] * quantity │
│          })                                              │
│                                                          │
│      # Step 4: Recalculate totals                        │
│      voice_cart["subtotal"] = sum(                       │
│          item["totalPrice"]                              │
│          for item in voice_cart["items"]                 │
│      )                                                   │
│      voice_cart["total"] = (                             │
│          voice_cart["subtotal"] +                        │
│          voice_cart["deliveryFee"]                       │
│      )                                                   │
│                                                          │
│      return {                                            │
│          "success": True,                                │
│          "message": f"Added {quantity} {item['name']}",  │
│          "cart": voice_cart,                             │
│          "subtotal": voice_cart["subtotal"],             │
│          "total": voice_cart["total"]                    │
│      }                                                   │
└──────────────────────────────────────────────────────────┘
       │
       ▼
┌──────────────────────────────────────────────────────────┐
│  Data Channel: Send cart to frontend                     │
│                                                          │
│  {                                                       │
│      "type": "tool_call",                                │
│      "tool_name": "quick_add_to_cart",                   │
│      "result": {                                         │
│          "success": true,                                │
│          "cart": {                                       │
│              "items": [...],                             │
│              "subtotal": 8.99,                           │
│              "deliveryFee": 2.99,                        │
│              "total": 11.98                              │
│          }                                               │
│      }                                                   │
│  }                                                       │
│                                                          │
│  Frontend updates cart display                           │
└──────────────────────────────────────────────────────────┘
       │
       ▼
┌──────────────────────────────────────────────────────────┐
│  Voice Response                                          │
│                                                          │
│  "I added New York cheesecake to your cart for $8.99.    │
│   Your current total is $11.98 including delivery."      │
└──────────────────────────────────────────────────────────┘
```

**Key Features**:
- **In-memory cart**: `voice_cart` global variable
- **Cart initialization**: Created on first item
- **Restaurant locking**: All items must be from same restaurant
- **Quantity updates**: Increments if item already in cart
- **Real-time totals**: Auto-calculates subtotal + delivery

---

## Action 4: View Cart (Voice)

```
User says: "What's in my cart?"
       │
       ▼
┌──────────────────────────────────────────────────────────┐
│  Tool: quick_view_cart (no parameters)                   │
└──────────────────────────────────────────────────────────┘
       │
       ▼
┌──────────────────────────────────────────────────────────┐
│  Database Layer (database.py)                            │
│                                                          │
│  def get_voice_cart():                                   │
│      global voice_cart                                   │
│                                                          │
│      if not voice_cart or not voice_cart.get("items"):   │
│          return {                                        │
│              "success": False,                           │
│              "message": "Your cart is empty"             │
│          }                                               │
│                                                          │
│      return {                                            │
│          "success": True,                                │
│          "cart": voice_cart,                             │
│          "restaurant": {                                 │
│              "name": voice_cart["restaurantName"]        │
│          },                                              │
│          "items": voice_cart["items"],                   │
│          "subtotal": voice_cart["subtotal"],             │
│          "deliveryFee": voice_cart["deliveryFee"],       │
│          "total": voice_cart["total"],                   │
│          "itemCount": sum(                               │
│              item["quantity"]                            │
│              for item in voice_cart["items"]             │
│          )                                               │
│      }                                                   │
└──────────────────────────────────────────────────────────┘
       │
       ▼
┌──────────────────────────────────────────────────────────┐
│  Voice Response                                          │
│                                                          │
│  "You have 1 item in your cart from Dessert Palace:      │
│   - New York Cheesecake x1 for $8.99.                    │
│   Your subtotal is $8.99, with $2.99 delivery,           │
│   your total is $11.98."                                 │
└──────────────────────────────────────────────────────────┘
```

**Key Feature**: No database query needed - reads in-memory cart

---

## Action 5: Checkout (Voice)

```
User says: "Place my order"
       │
       ▼
┌──────────────────────────────────────────────────────────┐
│  Tool: quick_checkout (no parameters)                    │
└──────────────────────────────────────────────────────────┘
       │
       ▼
┌──────────────────────────────────────────────────────────┐
│  Database Layer (database.py)                            │
│                                                          │
│  def checkout_cart():                                    │
│      global voice_cart                                   │
│                                                          │
│      if not voice_cart or not voice_cart.get("items"):   │
│          return {                                        │
│              "success": False,                           │
│              "message": "Cart is empty"                  │
│          }                                               │
│                                                          │
│      # Generate order number                             │
│      import time                                         │
│      order_number = f"VO{str(int(time.time()))[-6:]}"   │
│      # Example: "VO123456"                               │
│                                                          │
│      # Create order summary                              │
│      order_summary = {                                   │
│          "orderNumber": order_number,                    │
│          "success": True,                                │
│          "restaurant": {                                 │
│              "id": voice_cart["restaurantId"],           │
│              "name": voice_cart["restaurantName"]        │
│          },                                              │
│          "items": voice_cart["items"],                   │
│          "subtotal": voice_cart["subtotal"],             │
│          "deliveryFee": voice_cart["deliveryFee"],       │
│          "total": voice_cart["total"],                   │
│          "itemCount": len(voice_cart["items"])           │
│      }                                                   │
│                                                          │
│      # Clear cart after checkout                         │
│      voice_cart = None                                   │
│                                                          │
│      return {                                            │
│          "success": True,                                │
│          "orderId": order_number,                        │
│          "restaurant": order_summary["restaurant"],      │
│          "itemCount": order_summary["itemCount"],        │
│          "total": order_summary["total"],                │
│          "orderDetails": order_summary                   │
│      }                                                   │
└──────────────────────────────────────────────────────────┘
       │
       ▼
┌──────────────────────────────────────────────────────────┐
│  Data Channel: Send order confirmation to frontend       │
│                                                          │
│  {                                                       │
│      "type": "tool_call",                                │
│      "tool_name": "quick_checkout",                      │
│      "result": {                                         │
│          "success": true,                                │
│          "orderId": "VO123456",                          │
│          "total": 11.98,                                 │
│          "orderDetails": { ... }                         │
│      }                                                   │
│  }                                                       │
│                                                          │
│  Frontend displays order confirmation                    │
└──────────────────────────────────────────────────────────┘
       │
       ▼
┌──────────────────────────────────────────────────────────┐
│  Voice Response                                          │
│                                                          │
│  "Perfect! Your order has been placed. Your order        │
│   number is VO123456. The total is $11.98 and your       │
│   food will arrive in about 30 minutes from Dessert      │
│   Palace. Is there anything else I can help you with?"   │
└──────────────────────────────────────────────────────────┘
```

**Key Features**:
- **Order number generation**: `VO{timestamp}`
- **Cart clearing**: Resets for next order
- **No database persistence**: Demo mode (can be enhanced)

---

## Complete System Architecture Diagram

```
┌────────────────────────────────────────────────────────────────────┐
│                      USER INTERFACE (Browser)                      │
│  ═══════════════════════════════════════════════                   │
│                                                                    │
│  /app/food/concierge-agentserver/page.tsx                          │
│  • LiveKitRoom component (@livekit/components-react)               │
│  • Microphone input (getUserMedia)                                 │
│  • Audio output (Web Audio API)                                    │
│  • Data channel listener (for tool results)                        │
│  • Real-time card rendering                                        │
└────────────────────────┬───────────────────────────────────────────┘
                         │
                         │ 1. Request Token
                         ▼
┌────────────────────────────────────────────────────────────────────┐
│  TOKEN ENDPOINT (Next.js API Route)                                │
│  ═══════════════════════════════════════════                       │
│                                                                    │
│  /app/api/livekit-agentserver/token/route.ts                       │
│                                                                    │
│  1. RoomServiceClient.createRoom()                                 │
│     └─► Creates room: "food-concierge-agentserver-{timestamp}"     │
│                                                                    │
│  2. AgentDispatchClient.createDispatch()                           │
│     └─► Dispatches agent: "ubereats-food-concierge"                │
│                                                                    │
│  3. AccessToken.addGrant()                                         │
│     └─► Permissions: join, publish, subscribe                      │
│                                                                    │
│  4. Returns: { token, url, roomName }                              │
└────────────────────────┬───────────────────────────────────────────┘
                         │
                         │ 2. Connect with Token
                         ▼
┌────────────────────────────────────────────────────────────────────┐
│  LIVEKIT CLOUD INFRASTRUCTURE                                      │
│  ══════════════════════════════════                                │
│                                                                    │
│  • WebRTC Media Server (wss://your-project.livekit.cloud)        │
│  • Room Management                                                 │
│  • Participant Tracking                                            │
│  • Agent Dispatch System                                           │
│  • Media Routing (audio/video/data)                                │
│                                                                    │
│  Room: "food-concierge-agentserver-123456"                         │
│  Participants:                                                     │
│    - user-5678 (human)                                             │
│    - ubereats-food-concierge (agent)                               │
└────────────────────────┬───────────────────────────────────────────┘
                         │
                         │ 3. Agent Joins Room
                         ▼
┌────────────────────────────────────────────────────────────────────┐
│  PYTHON AGENT (AgentServer Pattern)                                │
│  ════════════════════════════════════                              │
│                                                                    │
│  /agents/food_concierge_agentserver.py                             │
│                                                                    │
│  @server.rtc_session(                                              │
│      agent_name="ubereats-food-concierge"                          │
│  )                                                                 │
│  async def food_concierge_agent(ctx: JobContext):                  │
│                                                                    │
│      # 1. Initialize user state                                    │
│      userdata = UserState()                                        │
│                                                                    │
│      # 2. Create agent session                                     │
│      session = AgentSession[UserState](                            │
│          userdata=userdata,                                        │
│          stt=inference.STT("deepgram/nova-3"),                     │
│          llm=inference.LLM("openai/gpt-4o-mini"),                  │
│          tts=openai.TTS(),                                         │
│          vad=silero.VAD.load(),                                    │
│          max_tool_steps=10                                         │
│      )                                                             │
│                                                                    │
│      # 3. Register event handlers                                  │
│      @session.on("user_speech_committed")                          │
│      def on_user_speech(transcript):                               │
│          log and publish to frontend                               │
│                                                                    │
│      @session.on("agent_speech_committed")                         │
│      def on_agent_speech(transcript):                              │
│          log and publish to frontend                               │
│                                                                    │
│      # 4. Start agent with tools                                   │
│      agent = FoodConciergeAgent(userdata=userdata)                 │
│      agent.tools = [                                               │
│          get_user_profile_tool,                                    │
│          find_food_item_tool,                                      │
│          find_restaurants_by_type_tool,                            │
│          get_restaurant_menu_tool,                                 │
│          fetch_menu_item_image_tool,                               │
│          quick_view_cart_tool,                                     │
│          quick_add_to_cart_tool,                                   │
│          remove_from_cart_tool,                                    │
│          update_cart_quantity_tool,                                │
│          quick_checkout_tool                                       │
│      ]                                                             │
│                                                                    │
│      await session.start(agent=agent, room=ctx.room)               │
│                                                                    │
│      # 5. Generate greeting                                        │
│      await session.generate_reply(                                 │
│          "Say: Hello! What are you in the mood for today?"         │
│      )                                                             │
└────────────┬───────────────────────────────────────────────────────┘
             │
             ├────────► 4. Tool Execution Flow
             │
             ▼
┌────────────────────────────────────────────────────────────────────┐
│  FUNCTION TOOL LAYER                                               │
│  ════════════════════════                                          │
│                                                                    │
│  Each tool follows this pattern:                                   │
│                                                                    │
│  @function_tool                                                    │
│  async def tool_name(                                              │
│      ctx: RunContext[UserState],                                   │
│      param: Annotated[Type, Field(description="...")]              │
│  ) -> str:                                                         │
│      """                                                           │
│      Tool description for LLM.                                     │
│      Examples of when to call this tool.                           │
│      """                                                           │
│      # 1. Log tool call                                            │
│      logger.info(f"🔧 Tool: {tool_name}({params})")               │
│                                                                    │
│      # 2. Call database layer                                      │
│      result = await database_function(params)                      │
│                                                                    │
│      # 3. Send data to frontend (if applicable)                    │
│      if ctx.userdata.local_participant:                            │
│          await ctx.userdata.local_participant.publish_data(        │
│              json.dumps({                                          │
│                  "type": "tool_call",                              │
│                  "tool_name": "...",                               │
│                  "result": result                                  │
│              }).encode(),                                          │
│              reliable=True                                         │
│          )                                                         │
│                                                                    │
│      # 4. Return text summary for voice                            │
│      return "Natural language summary of result..."                │
└────────────┬───────────────────────────────────────────────────────┘
             │
             ▼
┌────────────────────────────────────────────────────────────────────┐
│  DATABASE LAYER                                                    │
│  ════════════════                                                  │
│                                                                    │
│  /agents/database.py                                               │
│                                                                    │
│  Functions (mirror TypeScript tools):                              │
│                                                                    │
│  async def get_user_profile(profile_id):                           │
│      → Query fc_preferences                                        │
│                                                                    │
│  async def search_menu_items(query, max_results):                  │
│      → Query fc_menu_items with multi-word search                  │
│      → Rank by relevance                                           │
│      → Fetch images from Pexels if missing                         │
│                                                                    │
│  async def search_restaurants_by_cuisine(cuisine, max):            │
│      → Query fc_restaurants                                        │
│      → Search cuisine, cuisine_group, name                         │
│      → Fetch images from Pexels if missing                         │
│                                                                    │
│  async def get_restaurant_menu(restaurant_slug):                   │
│      → Query fc_menu_sections_with_items view                      │
│      → Return structured menu data                                 │
│                                                                    │
│  def add_to_voice_cart(item_name, quantity):                       │
│      → Search fc_menu_items for item                               │
│      → Initialize or update global voice_cart                      │
│      → Recalculate totals                                          │
│      → Return cart summary                                         │
│                                                                    │
│  def get_voice_cart():                                             │
│      → Return global voice_cart (in-memory)                        │
│                                                                    │
│  def remove_from_cart(item_name, quantity):                        │
│      → Update global voice_cart                                    │
│      → Recalculate totals                                          │
│                                                                    │
│  def update_cart_item_quantity(item_name, new_qty):                │
│      → Update global voice_cart                                    │
│      → Recalculate totals                                          │
│                                                                    │
│  def checkout_cart():                                              │
│      → Generate order number (VO{timestamp})                       │
│      → Create order summary                                        │
│      → Clear global voice_cart                                     │
│      → Return order details                                        │
└────────────┬───────────────────────────────────────────────────────┘
             │
             ▼
┌────────────────────────────────────────────────────────────────────┐
│  SUPABASE DATABASE                                                 │
│  ════════════════════                                              │
│                                                                    │
│  Python Client: supabase-py                                        │
│  Connection: Same credentials as TypeScript                        │
│                                                                    │
│  Tables:                                                           │
│  • fc_restaurants                                                  │
│  • fc_menu_sections                                                │
│  • fc_menu_items                                                   │
│  • fc_preferences                                                  │
│  • fc_menu_sections_with_items (view)                              │
│                                                                    │
│  Note: Cart operations use in-memory storage (voice_cart)          │
│        No fc_carts or fc_orders tables for voice demo              │
└────────────┬───────────────────────────────────────────────────────┘
             │
             ▼
┌────────────────────────────────────────────────────────────────────┐
│  EXTERNAL APIS                                                     │
│  ═══════════════                                                   │
│                                                                    │
│  • OpenAI API (via LiveKit inference layer)                        │
│    - GPT-4o-mini for LLM                                           │
│    - Function calling for tools                                    │
│    - TTS for voice synthesis                                       │
│                                                                    │
│  • Deepgram API (via LiveKit inference layer)                      │
│    - Nova-3 model for STT                                          │
│    - Real-time transcription                                       │
│                                                                    │
│  • Pexels API (direct)                                             │
│    - Image search for food items                                   │
│    - Image search for restaurants                                  │
└────────────────────────────────────────────────────────────────────┘
```

---

## LiveKit AgentServer Key Concepts

### 1. AgentServer Pattern

```python
from livekit.agents import AgentServer

server = AgentServer()

@server.rtc_session(
    on_session_end=on_session_end,
    agent_name="ubereats-food-concierge"  # Unique identifier
)
async def food_concierge_agent(ctx: JobContext) -> None:
    """
    Main agent entry point.
    
    Called automatically when:
    - User connects to room
    - Agent is dispatched by name
    
    Context provides:
    - ctx.room: LiveKit room instance
    - ctx.room.local_participant: Agent's participant
    - ctx.room.remote_participants: User participants
    """
    # Setup agent session...
    await session.start(agent=agent, room=ctx.room)
```

**How it works:**
1. Agent runs as a server process: `python food_concierge_agentserver.py dev`
2. Registers with LiveKit Cloud
3. Listens for dispatch events
4. Auto-joins rooms when dispatched by name
5. Handles WebRTC media streams

### 2. Typed UserState

```python
from dataclasses import dataclass

@dataclass
class UserState:
    """Session-specific state for each user"""
    user_id: str | None = None
    cart_id: str | None = None
    profile: dict | None = None
    order_count: int = 0
    local_participant: any = None  # For data channel
```

**Purpose:**
- Maintain state across tool calls
- Access room/participant info
- Store user context
- Pass data between tools

### 3. AgentSession Configuration

```python
from livekit.agents import AgentSession, inference
from livekit.plugins import openai, silero

session = AgentSession[UserState](
    userdata=userdata,
    
    # Speech-to-Text
    stt=inference.STT(
        "deepgram/nova-3",  # Model identifier
        language="en"        # Language code
    ),
    
    # Language Model
    llm=inference.LLM("openai/gpt-4o-mini"),
    
    # Text-to-Speech (OpenAI plugin, not inference)
    tts=openai.TTS(),
    
    # Voice Activity Detection
    vad=silero.VAD.load(),
    
    # Safety: Maximum tool calls per turn
    max_tool_steps=10
)
```

**Inference Layer:**
- Unified API for STT/LLM across providers
- Handles model connections automatically
- Manages retries and errors
- Supports multiple providers (Deepgram, OpenAI, etc.)

### 4. Function Tools

```python
from livekit.agents import function_tool
from typing import Annotated
from pydantic import Field

@function_tool
async def my_tool(
    ctx: RunContext[UserState],
    param: Annotated[
        str,
        Field(description="Parameter description for LLM")
    ]
) -> str:
    """
    Tool description that LLM reads.
    
    Explain when to call this tool.
    Provide examples.
    """
    # Access user state
    user_id = ctx.userdata.user_id
    
    # Perform operation
    result = await database_operation(param)
    
    # Send data to frontend
    if ctx.userdata.local_participant:
        await ctx.userdata.local_participant.publish_data(
            json.dumps(result).encode(),
            reliable=True
        )
    
    # Return text for voice response
    return "Natural language summary"
```

**Key Points:**
- `@function_tool` decorator registers tool with agent
- `RunContext[UserState]` provides typed access to state
- `Annotated[Type, Field(...)]` adds descriptions for LLM
- Docstring describes tool purpose
- Must return string (for voice synthesis)
- Can send structured data via data channel

### 5. Event System

```python
@session.on("user_speech_committed")
def on_user_speech(transcript: str):
    """Called when STT finalizes user speech"""
    logger.info(f"User said: {transcript}")
    # Send to frontend for display

@session.on("agent_speech_committed")
def on_agent_speech(transcript: str):
    """Called when agent response is generated"""
    logger.info(f"Agent saying: {transcript}")
    # Send to frontend for display

@session.on("function_calls_collected")
def on_function_calls(calls: list):
    """Called when LLM decides to call tools"""
    for call in calls:
        logger.info(f"Tool: {call.function_call.name}")

@session.on("function_calls_finished")
def on_function_results(results: list):
    """Called when tools finish executing"""
    for result in results:
        logger.info(f"Result: {result.result[:200]}")
```

**Events provide visibility:**
- Track conversation flow
- Debug tool calls
- Send updates to frontend
- Log for analytics

---

## Data Flow: Voice Cart Management

### In-Memory Cart (voice_cart)

```python
# Global variable in database.py
voice_cart: Optional[Dict[str, Any]] = None

# Structure:
voice_cart = {
    "restaurantId": "uuid",
    "restaurantSlug": "dessert-palace",
    "restaurantName": "Dessert Palace",
    "items": [
        {
            "id": "item-uuid",
            "slug": "new-york-cheesecake",
            "name": "New York Cheesecake",
            "quantity": 2,
            "basePrice": 8.99,
            "totalPrice": 17.98
        }
    ],
    "subtotal": 17.98,
    "deliveryFee": 2.99,
    "total": 20.97
}
```

**Why in-memory?**
- Faster for voice interactions (no DB round-trips)
- Simpler state management
- Demo/prototype focused
- Can be enhanced to use fc_carts table

**Architecture:**
```
User: "Add cheesecake"
       │
       ▼
Tool: quick_add_to_cart_tool()
       │
       ▼
Database: add_to_voice_cart()
       │
       ├─► Query fc_menu_items (find item)
       │
       ├─► Update global voice_cart variable
       │       ├─► Initialize if None
       │       ├─► Add/update item
       │       └─► Recalculate totals
       │
       └─► Return cart summary
```

---

## Comparison: AI SDK vs LiveKit

### Feature Comparison

| Feature | AI SDK (Chat) | LiveKit (Voice) |
|---------|---------------|-----------------|
| **Input Format** | Text (JSON) | Audio (WebRTC) |
| **Output Format** | Text stream | Audio stream |
| **Language** | TypeScript | Python |
| **Runtime** | Node.js/Edge | Python process |
| **State Storage** | Message history | UserState + in-memory |
| **Tool Definition** | `tool()` function | `@function_tool` decorator |
| **Database** | Supabase (TypeScript) | Supabase (Python) |
| **Cart Storage** | fc_carts table | In-memory (voice_cart) |
| **Real-time** | HTTP streaming | WebRTC duplex |
| **Latency** | ~500ms-2s | ~300ms-1s (voice optimized) |
| **Complexity** | Medium | Higher (media handling) |

### Architecture Comparison

```
AI SDK FLOW:
User types → HTTP POST → AI SDK route → OpenAI →
Tool execution → Database → OpenAI response →
Streaming text → Frontend display

LIVEKIT FLOW:
User speaks → WebRTC audio → LiveKit Cloud →
Python Agent → Deepgram STT → OpenAI LLM →
Tool execution → Database → OpenAI TTS →
WebRTC audio → Frontend plays
```

### When to Use Each

**AI SDK (Chat):**
- Text-based interfaces
- Complex multi-turn conversations
- Rich text formatting needed
- Lower latency requirements
- Easier debugging
- Better for complex workflows

**LiveKit (Voice):**
- Voice-first experiences
- Hands-free interactions
- Real-time conversations
- Natural language priority
- Accessibility (vision-impaired)
- Automotive/IoT applications

---

## Environment Configuration

### Required Environment Variables

```bash
# .env.local

# LiveKit Credentials
LIVEKIT_URL=wss://your-project.livekit.cloud
LIVEKIT_API_KEY=APIxxxxx
LIVEKIT_API_SECRET=xxxxxxxxxx

# OpenAI (for LLM and TTS)
OPENAI_API_KEY=sk-proj-xxxxxx

# Supabase
SUPABASE_URL=https://xxx.supabase.co
SUPABASE_SERVICE_ROLE_KEY=xxxxx

# Demo Profile
DEMO_PROFILE_ID=00000000-0000-0000-0000-0000000000fc

# Optional: Pexels for images
PEXELS_API_KEY=xxxxx
```

### Python Requirements

```txt
# agents/requirements.txt

livekit-agents>=1.4.1
livekit-plugins-openai
livekit-plugins-deepgram
livekit-plugins-silero
supabase
python-dotenv
httpx
```

---

## Running the Agent

### Development Mode

```bash
# Terminal 1: Start Next.js frontend
npm run dev

# Terminal 2: Start Python agent
cd agents
python food_concierge_agentserver.py dev
```

### Agent Logs

```
✅ Database client initialized
   Supabase URL: http://127.0.0.1:54321
   Demo Profile: 00000000-0000-0000-0000-0000000000fc
   Pexels API: ✓ Configured

🚀 Agent starting for room: food-concierge-agentserver-123456
🔄 Voice cart reset for new session
✅ Agent connected, local participant stored
🎙️ Generating greeting...
✅ Agent session started with greeting

🎤 USER SAID: 'Show me Thai restaurants'
🔧 TOOL CALLED: find_restaurants_by_type({"cuisine_type": "thai"})
🔧 Tool: find_restaurants_by_type(cuisine_type='thai')
   ✅ Found 3 restaurants
   📤 Sent 3 restaurants to frontend
✅ TOOL RESULT: Found 3 Thai restaurants:...
🤖 AGENT SAYING: 'I found 3 great Thai restaurants for you...'
```

---

## Debugging Tips

### 1. Check Agent Connection

```bash
# Agent should show:
🚀 Agent starting for room: food-concierge-agentserver-xxxxx
✅ Agent connected, local participant stored
```

### 2. Monitor Tool Calls

```bash
# Look for:
🔧 TOOL CALLED: tool_name({"param": "value"})
   ✅ Success message
   📤 Sent data to frontend
```

### 3. Check Data Channel

```typescript
// Frontend: Listen for tool results
room.on('data-received', (payload) => {
  const data = JSON.parse(payload);
  if (data.type === 'tool_call') {
    console.log('Tool result:', data.tool_name, data.result);
  }
});
```

### 4. Verify Database Connection

```python
# database.py logs:
✅ Database client initialized
   Supabase URL: https://xxx.supabase.co
```

### 5. Common Issues

| Issue | Cause | Solution |
|-------|-------|----------|
| Agent doesn't join | Wrong agent name | Check `agent_name` matches dispatch |
| No audio | Permissions | Allow microphone in browser |
| Tool not called | Poor description | Improve tool docstrings |
| Database error | Wrong credentials | Check .env.local |
| Cart not working | voice_cart None | Check reset_voice_cart() called |

---

## Performance Optimization

### 1. STT/TTS Selection

```python
# Faster STT:
stt=inference.STT("deepgram/nova-2")  # Slightly faster

# Better quality STT:
stt=inference.STT("deepgram/nova-3")  # More accurate

# Faster TTS:
tts=inference.TTS("elevenlabs/eleven_turbo_v2")

# Better quality TTS:
tts=openai.TTS(voice="alloy")  # Natural sounding
```

### 2. Tool Optimization

```python
# Limit results for voice
max_results = 3  # Don't overwhelm voice UX

# Parallel data fetching
import asyncio
results = await asyncio.gather(
    search_menu_items(query1),
    search_restaurants(query2)
)

# Cache frequent queries
from functools import lru_cache

@lru_cache(maxsize=100)
def get_restaurant_by_slug(slug):
    # Cache results
```

### 3. Database Optimization

```python
# Use indexes (in Supabase)
CREATE INDEX idx_menu_items_name_trgm 
ON fc_menu_items USING gin(name gin_trgm_ops);

# Limit queries
.limit(max_results)

# Select only needed fields
.select("id, name, base_price")  # Not SELECT *
```

---

## Summary

### LiveKit AgentServer Powers:
- ✅ Real-time voice conversations (WebRTC)
- ✅ Automatic STT/TTS pipeline
- ✅ Function tool execution
- ✅ Typed state management (UserState)
- ✅ Event system for visibility
- ✅ Data channel for structured results
- ✅ Python-based extensibility

### Key Services:
- **LiveKit Cloud**: WebRTC infrastructure, room management, agent dispatch
- **Deepgram Nova-3**: Speech-to-text (STT)
- **OpenAI GPT-4o-mini**: Language model (LLM) with function calling
- **OpenAI TTS**: Text-to-speech (natural voices)
- **Supabase**: Database for food data
- **Pexels**: Image search for food/restaurants

### The Voice Flow:
```
User Voice → STT → LLM → Tool Selection → 
Database Query → LLM Response → TTS → User Hears
```

### Advantages:
- **Natural**: Voice-first experience
- **Fast**: ~500ms voice-to-response
- **Flexible**: Python extensibility
- **Integrated**: Data channel + audio
- **Scalable**: LiveKit Cloud handles media

---

## Next Steps

- Explore system prompt engineering for better responses
- Add conversation memory (store in UserState)
- Enhance cart with database persistence
- Add payment integration at checkout
- Implement order tracking
- Add multi-language support
- Optimize for mobile/automotive

**Files to Explore:**
- `/agents/food_concierge_agentserver.py` - Main agent
- `/agents/database.py` - Database layer
- `/app/api/livekit-agentserver/token/route.ts` - Token endpoint
- `/app/food/concierge-agentserver/page.tsx` - Frontend

---

## Resources

- [LiveKit Agents Documentation](https://docs.livekit.io/agents/)
- [LiveKit Python SDK](https://github.com/livekit/python-sdks)
- [AgentServer Pattern Examples](https://github.com/livekit/agents/tree/main/examples)
- [Supabase Python Client](https://supabase.com/docs/reference/python)
- [OpenAI Function Calling](https://platform.openai.com/docs/guides/function-calling)

